...
/*   Updated: 2026/04/17 14:08:25 by vlad             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_free_data(t_data *data)
{
	...
	while (count < data->n_philosophers)
	{
		...
		pthread_mutex_destroy(&data->philosophers[count].last_meal_time_mutex);
		count++;
	}
...
int	main(int ac, char **av)
{
	...
	if (ft_create_spectator(data) == 1)
		return (ft_putstr_fd("Error: Failed to create spectator thread\n", STDERR_FILENO),
			free(data->philosophers), free(data->forks), free(data), 1);
